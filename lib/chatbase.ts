...
/**
 * Chat with Chatbase using conversationId - Chatbase manages the history automatically!
 * This is the preferred method as it ensures conversation continuity.
 */
export async function queryChatbaseWithConversation(
  ...
  // If we have a conversationId, Chatbase will continue that conversation
  if (conversationId) {
    body.conversationId = conversationId;
  }
  
  console.log('[v0] Chatbase request body:', JSON.stringify(body, null, 2));
  ...
  try {
    ...
    console.log('[v0] Chatbase response:', JSON.stringify(data, null, 2));
    
    return {
      text: data.text || data.message || data.answer || 'No response from Chatbase',
